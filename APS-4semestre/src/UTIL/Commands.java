...
public class Commands extends DbConnection {
	...
	public ArrayList<BooksAuthors> searchBooksAuthors(String name) {
		...
		int author_id = searchAuthor(name).author_id;

		final String query = "SELECT b.title, p.url, b.price  FROM booksauthors ba "
		...
		try (Connection connection = getConexaoMySQL()) {
			...
			while (rs.next()) {
				...
				booksAuthors.setUrl(rs.getString(2));
				booksAuthors.setPrice(rs.getDouble(3));
